...
inv_error_t inv_register_load_store(inv_error_t (*load_func)(const unsigned char *data),
                                    inv_error_t (*save_func)(unsigned char *data), unsigned char size, unsigned int key)
{
    int kk;
...
* @return Returns INV_SUCCESS if successful or an error code if not.
*/
inv_error_t inv_get_mpl_state_size(unsigned char *size)
{
    *size = ds.total_size;
    return INV_SUCCESS;
}
...
* @return Returns INV_SUCCESS if successful or an error code if not.
*/
inv_error_t inv_load_mpl_states(const unsigned char *data, unsigned char length)
{
...
* @return Returns INV_SUCCESS if successful or an error code if not.
*/
inv_error_t inv_save_mpl_states(unsigned char *data, unsigned char sz)
{
    unsigned char *cur;
    int kk;
    struct data_header_t *hd;
